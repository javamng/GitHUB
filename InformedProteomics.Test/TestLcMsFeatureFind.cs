...
namespace InformedProteomics.Test
{
    ...
    public class TestLcMsFeatureFind
    {
        ...
        public void TestLcMsFeatureFinder()
        {
            ...
            var maxSearchMassBin = featureFinder.Comparer.GetBinNumber(3400);
        ...
        public void CollectTrainingSet()
        {
            ...
            foreach (var dataset in rawFileLists)
            {
                ...
                var outFileName = string.Format(@"{0}\{1}.trainset.tsv", outFileFolder,
        ...
        public void TestFeatureFindingWithMassBinning()
        {
            ...
            foreach (var dataset in rawFileLists)
            {
                ...
                var featureResult = string.Format(@"{0}\{1}.ms1ft", idFileFolder,
                    Path.GetFileNameWithoutExtension(dataset));
                var targetStatWriter =
                    new StreamWriter(string.Format(@"D:\MassSpecFiles\training\stats\{0}.tsv",
                        Path.GetFileNameWithoutExtension(dataset)));
                var decoyStatWriter =
                    new StreamWriter(string.Format(@"D:\MassSpecFiles\training\stats\{0}_decoy.tsv",
                        Path.GetFileNameWithoutExtension(dataset)));
                ...
                for (var i = 0; i < tsvParser.NumData; i++)
                {
                    ...
                    foreach (var bi in binNumList)
                    {
                        ...
                        {
                            var scans2 = Enumerable.Range(feature.MinScanNum,
                                feature.MaxScanNum - feature.MinScanNum + 1);
